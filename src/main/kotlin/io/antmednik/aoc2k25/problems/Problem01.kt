 ...
 *
 * "Due to new security protocols, the password is locked in the safe below. Please see the attached document for
 * the new combination."
 *
 ...
 *
 * So, if the dial were pointing at 11, a rotation of R8 would cause the dial to point at 19. After that,
 * a rotation of L19 would cause it to point at 0.
 *
 ...
 * count the number of times any click causes the dial to point at 0, regardless of whether it happens
 ...
 * In this example, the dial points at 0 three times at the end of a rotation, plus three more times during
 * a rotation. So, in this example, the new password would be 6.
 *
 * Be careful: if the dial were pointing at 50, a single rotation like R1000 would cause the dial to point at
...
class Problem01 {
    fun solvePart1(): Int {
        return InputFileSequence("problem01.txt").use { ife ->
            part1(INITIAL_DIAL, ife)
        }
    }

    fun solvePart2(): Int {
        return InputFileSequence("problem01.txt").use { ife ->
            part2(INITIAL_DIAL, ife)
        }
    }

    internal fun part1(initialDial: Int, ops: Sequence<String>): Int {
        var dial = initialDial
        var pCounter = 0
        ops.forEach {
            dial = nextDial(dial, it)
            if (dial == 0) {
                pCounter++
            }
        }
    ...
    private fun nextDial(dial: Int, op: String): Int {
        var step = Integer.parseInt(op.slice(1..<op.length))
        step %= MAX_DIAL
        return if (isLeftTurn(op)) {
            var diff = dial - step
            if (diff < 0) {
                diff += MAX_DIAL
            }
    ...
    internal fun part2(initialDial: Int, ops: Sequence<String>): Int {
        var dial = initialDial
        var pCounter = 0
        ops.forEach {
            val pair = nextDial2(dial, it)
            dial = pair.first
            if (dial == 0) {
                pCounter++
            }
            pCounter += pair.second
        }
        return pCounter
    }

    private fun nextDial2(dial: Int, op: String): Pair<Int, Int> {
        var step = Integer.parseInt(op.slice(1..<op.length))
        var zeros = step / MAX_DIAL
        step %= MAX_DIAL
        var newDial: Int
        if (isLeftTurn(op)) {
            newDial = dial - step
            ...
            newDial = dial + step
            if (newDial > MAX_DIAL && dial != 0) {
                zeros++
            }
            newDial %= MAX_DIAL
        }
        return Pair(newDial, zeros)
    }

    private companion object {
        const val INITIAL_DIAL = 50
        const val MAX_DIAL = 100

        fun isLeftTurn(op: String) = op[0] == 'L'
    }
}