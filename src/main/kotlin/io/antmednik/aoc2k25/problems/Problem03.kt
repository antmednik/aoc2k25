 ...
 * The batteries are each labeled with their joltage rating, a value from 1 to 9. You make a note of their joltage
 ...
 * of the system and hits the big red "joltage limit safety override" button. You lose count of the number of times
...
class Problem03 {
    ...
    internal fun findMaxInBank(bank: String): Int {
        var leftMax = bank[0]
        var rightMax = bank[1]
        var maxJoltage = joltage(leftMax, rightMax)
        var leftCandidate = bank[2]
        for (i in 3..<bank.length) {
            val battery = bank[i]
            // current max improved
            if (maxJoltage < joltage(leftMax, battery)) {
                rightMax = battery
                maxJoltage = joltage(leftMax, rightMax)
            }
            if (maxJoltage < joltage(leftCandidate, battery)) {
                leftMax = leftCandidate
                rightMax = battery
                maxJoltage = joltage(leftMax, rightMax)
            }
            if (leftCandidate < battery) {
                leftCandidate = battery
            }
        }
        return maxJoltage
    }
