 ...
 *
 * --- Part Two ---
 * The clerk quickly discovers that there are still invalid IDs in the ranges in your list. Maybe the young Elf
 * was doing other silly patterns as well?
 *
...
class Problem02 {
    ...
    fun solvePart2(): Long {
        return InputFileSequence("problem02.txt").use { ife ->
            ife.first().split(',').sumOf {
                val rawRange = it.split("-")
                sumInvalidIds2(rawRange[0], rawRange[1])
            }
        }
    }
     ...
     * retval = 121212
     */
    private fun getFull2(sequence: Long, times: Int): Long {
        var multiplier = 1L
        var tmp = sequence
        while (tmp > 0) {
            multiplier *= 10
    ...
    private fun subGroupExclusiveEnd(subGroupLen: Int): Long {
        var retval = 1L
        repeat(subGroupLen) { retval *= 10}
        return retval
    }

    internal fun sumInvalidIds2(startId: String, endId: String): Long {
        var groupLen = startId.length
        val endIdAsLong = endId.toLong()
        var groupStart = startId.toLong()
        var groupEnd = if (endId.length == groupLen) endIdAsLong else "9".repeat(groupLen).toLong()
        var invalidIds = mutableSetOf<Long>()
        while (groupLen <= endId.length) {
            collectInvalidIdsInGroup(groupStart, groupEnd, groupLen, invalidIds)
            groupLen++
            groupStart = ("1" + "0".repeat(groupLen-1)).toLong()
            if (endId.length == groupLen) {
                groupEnd = endIdAsLong
            } else {
                groupEnd = groupEnd * 10 + 9
            }
        }
        return invalidIds.sum()
    }

    internal fun collectInvalidIdsInGroup(groupStart: Long, groupEnd: Long, groupDigitsCount: Int, invalidIds: MutableSet<Long>) {
        for (subGroupLen in divisors[groupDigitsCount] ?: throw IllegalStateException("divisors unset for $groupDigitsCount")) {
            val subGroupsCount = groupDigitsCount / subGroupLen
            var subGroup = groupStart.toString().substring(0, subGroupLen).toLong()
            val candidate = getFull2(subGroup, subGroupsCount)
            if (candidate in groupStart..groupEnd) {
                invalidIds.add(candidate)
            }
            subGroup++
            val sgee = subGroupExclusiveEnd(subGroupLen)
            while (subGroup < sgee) {
                val candidate = getFull2(subGroup, subGroupsCount)
                if (candidate <= groupEnd) {
                    invalidIds.add(candidate)
                } else {
                    break
                }
